    ...
    handleToken(matches[1], (err, account) => {
      ...
      async.waterfall([
        ...
        function parseReq(req, cb) {
          var data = req.body;
          var results = {
            corp: data.corporation.id_str === config.EVE_CORP_ID,
            alliance: false/*data.corporation.id === config.EVE_CORP_ID*/
          };
          if (!results.corp && !results.alliance) {
            return cb("no roles granted, invalid corp or alliance.");
          }
          cb(null, results);
        },
        function addCorpRole(user, cb) {
          debug("Starting corp role");
          if(user.corp) {
        ...
        if(err) {
          debug("Err handling discord auth");
